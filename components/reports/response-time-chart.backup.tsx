...
export function ResponseTimeChart({ dateRange }: ResponseTimeChartProps) {
  ...
  const loadChartData = async () => {
    try {
      ...
      const response = await apiClient.getReportsResponseTimes(fromDate.toISOString(), toDate.toISOString(), "day") as any[]
      setData(response)
    } catch (err) {
          ...
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={data} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
              ...
              <Line
                ...
                stroke="var(--chart-1)"
                strokeWidth={2}
                dot={{ fill: "var(--chart-1)", stroke: "var(--chart-1)", strokeWidth: 2, r: 4 }}
                activeDot={{ r: 5, stroke: "var(--chart-1)" }}
              ...
              <Line
                ...
                stroke="var(--chart-2)"
                strokeWidth={2}
                dot={{ fill: "var(--chart-2)", stroke: "var(--chart-2)", strokeWidth: 2, r: 4 }}
                activeDot={{ r: 5, stroke: "var(--chart-2)" }}
